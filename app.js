    ...
    recurring.forEach((flow) => {
        ...
        item.innerHTML = `
            <div style="display:flex; align-items:center; gap:10px;">
                ...
                <div>
                    <div style="font-weight:bold;">${flow.title}</div>
                    <div style="font-size:0.8rem; color:var(--text-muted);">Scheduled</div>
                </div>
            </div>
            <button class="btn-sm btn-gold">Start</button>
        `;
...
function renderFlowBuilderSteps() {
    ...
    currentFlowBuilderSteps.forEach((step, index) => {
        ...
        el.innerHTML = `
            ...
            <div style="display:grid; grid-template-columns: 1fr 1fr; gap:10px;">
                <div>
                    <label style="font-size:0.8rem; color:#888">Image Upload</label>
                    <input type="file" class="input" accept="image/*" onchange="handleStepImageUpload(${index}, this)">
                    ${step.image ? `<div style="font-size:0.7rem; color:var(--gold-primary); margin-top:5px;">Image Loaded</div>` : ''}
                </div>
                <div>
...
window.saveFlow = () => {
    ...
    if (currentEditingFlowId) {
        const flow = AppState.flows.find(f => f.id === currentEditingFlowId);
        if (flow) {
            flow.title = title;
...
function renderTemplates() {
    ...
    FLOW_TEMPLATES.forEach((tmpl, index) => {
        ...
        card.innerHTML = `
            <div class="flow-image" style="background: linear-gradient(45deg, #222, #333)"></div>
            <div class="flow-content">
...
function renderRunnerStep() {
    ...
    content.innerHTML = `
        ...
        <p style="color:#888; margin-top:10px;">${
            ...
            (step.type === 'stopwatch' ? 'Time your session' : 'Read and internalize'))
        }</p>
    `;
...
function initWorkings(filterStatus = 'active') {
    ...
    if (list) {
        AppState.workings.filter(w => w.status === 'active').forEach(w => {
            ...
            item.innerHTML = `
                <span>${w.name}</span>
                <button class="btn-sm btn-gold" onclick="doWorkingDaily(${w.id})">Do</button>
            `;
            list.appendChild(item);
        });
    }
...
function runBreathingLoop() {
    ...
    const loop = () => {
        if (!document.getElementById('breathingOverlay').classList.contains('active')) return;
        
        if (phase === 'inhale') {
            text.textContent = "Inhale";
            circle.style.transform = "scale(2.5)";
            circle.style.opacity = "1";
            AppState.breathingInterval = setTimeout(() => { phase = 'hold1'; loop(); }, 4000);
        } else if (phase === 'hold1') {
            text.textContent = "Hold";
            AppState.breathingInterval = setTimeout(() => { phase = 'exhale'; loop(); }, 4000);
        } else if (phase === 'exhale') {
            text.textContent = "Exhale";
            circle.style.transform = "scale(1)";
            circle.style.opacity = "0.5";
            AppState.breathingInterval = setTimeout(() => { phase = 'hold2'; loop(); }, 4000);
        } else if (phase === 'hold2') {
            text.textContent = "Hold";
            AppState.breathingInterval = setTimeout(() => { phase = 'inhale'; loop(); }, 4000);
        }
    };
    loop();
}
    ...
    AppState.journal.forEach(entry => {
        ...
        el.innerHTML = `
            ...
            <div class="card-content">
                <p>${entry.content}</p>
                <div style="margin-top:10px; font-size:0.8rem; color:var(--gold-dim)">Mood: ${entry.mood}</div>
            </div>
        `;
        list.appendChild(el);
    });
...
window.openMasteryModal = () => window.openModal('masteryModal');
...
window.saveMastery = () => {
    ...
    const goal = document.getElementById('masteryGoal').value === 'custom' 
        ? parseInt(document.getElementById('customHours').value) 
        : parseInt(document.getElementById('masteryGoal').value);
...
window.openMasteryLogModal = (id) => {
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        <label>Amount (<span id="masteryLogUnit"></span>)</label>
                        <input type="number" id="masteryLogAmount" class="input" value="1" step="0.1">
                        <p style="font-size:0.7rem; color:#888; margin-top:5px;">Use negative numbers to subtract time/reps.</p>
                    </div>
                    <div class="form-group">
    ...
    const unitEl = modal.querySelector('#masteryLogUnit');
    if (unitEl) unitEl.textContent = unitLabel;
    
    const amountEl = modal.querySelector('#masteryLogAmount');
    ...
    if (confirmBtn) {
        confirmBtn.onclick = () => {
            const amountInput = document.getElementById('masteryLogAmount');
            const amount = amountInput ? parseFloat(amountInput.value) : 0;
            if (amount !== 0) { // Allow negative numbers
...
function renderCalendar() {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
        ...
        // Check for events (Schedule + Journal + Recurring)
        // Note: new Date(dateStr) might be off by timezone, so we construct carefully or use getFlowsForDate which expects Date object
        // But getFlowsForDate uses .getDay() which is local time. 
        // Let's construct date at noon to avoid timezone shifts
        ...
        if (recurring.length > 0 || oneOffs.length > 0 || journalEntries.length > 0) {
            ...
            indicator.style.margin = '5px auto 0';
...
function initAstro() {
    ...
    // Chaldean Order: Saturn(6), Jupiter(5), Mars(4), Sun(0), Venus(2), Mercury(3), Moon(1)
    const chaldeanOrder = [6, 5, 4, 0, 2, 3, 1]; 
    
    // Day Rulers (Standard): Sun(0), Moon(1), Mars(4), Mercury(3), Jupiter(5), Venus(2), Saturn(6)
    const dayRulers = [0, 1, 4, 3, 5, 2, 6]; 
    ...
    document.getElementById('currentPlanet').textContent = planetName + (isDay ? " (Day)" : " (Night)");
}
